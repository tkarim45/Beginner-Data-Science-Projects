"""
Product Recommendation System — Utility Functions
Implicit-feedback collaborative filtering on the UCI Online Retail dataset:
the signal is how many units of a product a customer purchased (no explicit ratings).
...
Two models are compared against a popularity baseline:
  - Item-based CF       : cosine similarity between products on the purchase matrix
...
UCOL, ICOL, WCOL = "CustomerID", "StockCode", "Quantity"
...
def load_interactions(path="data/transactions.csv"):
    """Load implicit interactions as user / item / weight (purchase quantity), aggregated per customer-product."""
    df = pd.read_csv(path)
    df = df.rename(columns={UCOL: "user", ICOL: "item", WCOL: "weight"})
    return df.groupby(["user", "item"], as_index=False)["weight"].sum()


def load_items(path="data/transactions.csv", id_col="StockCode", name_col="Description"):
    """Load product-code → description lookup (most common description per code)."""
    df = pd.read_csv(path)
    return df.groupby(id_col)[name_col].agg(lambda s: s.mode().iloc[0] if not s.mode().empty else "")
...
def build_matrix(df, log=True):
    """
    Pivot to a (users × items) implicit matrix; missing = 0. Purchase quantities span several
